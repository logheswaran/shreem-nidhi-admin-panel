...
async function run() {
  const { data: chits } = await supabase.from('chits').select('id, name, current_month');
  console.log('Valid Chits in DB:', chits);
  
  const { data: contributions } = await supabase.from('contributions').select('id, chit_id, month_number, amount_due');
  console.log('Contributions in DB:', contributions);
}
