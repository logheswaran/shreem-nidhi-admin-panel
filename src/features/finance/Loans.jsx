...
const Loans = () => {
  ...
  const columns = [
    { 
      ...
      render: (row) => (
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center font-bold text-sm text-blue-600 border border-blue-100 shadow-inner">
    ...
    { 
      ...
      render: (row) => (
        <div>
          <span className="font-bold text-brand-navy block leading-none mb-1">₹{Number(row.amount).toLocaleString()} Capital</span>
    ...
    { 
      ...
      render: (row) => (
        <div className="flex flex-col">
          <span className="font-black text-red-600 leading-none mb-1">₹{Number(row.balance).toLocaleString()}</span>
          <span className="text-[9px] font-black uppercase tracking-widest text-red-600/40">Current Liability</span>
        </div>
      ) 
    },
    ...
    { 
      ...
      render: (row) => row.status === 'active' && (
        <button 
          ...
          className="bg-brand-navy text-white text-[10px] font-black uppercase tracking-widest px-6 py-2.5 rounded-full hover:bg-brand-navy-light transition-all shadow-md active:scale-95 flex items-center gap-2"
        >
  ...
  return (
    <div className="animate-in fade-in duration-700">
      <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h2 className="text-4xl font-headline font-bold text-brand-navy">Credit Authorization</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        <div className="bg-white p-8 rounded-[2.5rem] border border-brand-gold/5 flex items-center gap-5 shadow-sm transform hover:-translate-y-1 transition-all">
           <div className="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center shadow-inner"><BankIcon className="text-blue-600 w-7 h-7"/></div>
           <div>
             <p className="text-2xl font-headline font-bold text-brand-navy leading-none">₹{loans.reduce((s,l) => s + (l.status === 'active' ? Number(l.balance) : 0), 0).toLocaleString()}</p>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-brand-gold/5 flex items-center gap-5 shadow-sm transform hover:-translate-y-1 transition-all">
           ...
           <div>
             <p className="text-2xl font-headline font-bold text-brand-navy leading-none">{loans.filter(l => l.status === 'active').length}</p>
             <p className="text-[10px] font-black uppercase tracking-[0.15em] text-brand-gold/60 mt-2">Circulating Accounts</p>
           </div>
        </div>
        <div className="bg-white p-8 rounded-[2.5rem] border border-brand-gold/5 flex items-center gap-5 shadow-sm transform hover:-translate-y-1 transition-all">
           <div className="w-14 h-14 rounded-2xl bg-green-50 flex items-center justify-center shadow-inner"><CheckCircle2 className="text-green-600 w-7 h-7"/></div>
           <div>
             <p className="text-2xl font-headline font-bold text-brand-navy leading-none">{loans.filter(l => l.status === 'closed').length}</p>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Underwriting Authorization">
        <form onSubmit={handleIssueLoan} className="space-y-10">
           <div className="p-10 bg-brand-ivory rounded-[3rem] border border-brand-gold/10 shadow-inner">
              <div className="space-y-8">
                <div className="space-y-3">
                  ...
                  <select 
                    required
                    className="w-full bg-white border-2 border-brand-gold/5 rounded-3xl p-5 text-sm font-body font-bold text-brand-navy focus:ring-4 focus:ring-brand-gold/10 focus:border-brand-gold/40 focus:outline-none transition-all shadow-sm"
           ...
           <div className="bg-brand-navy/5 p-8 rounded-[2.5rem] border border-brand-navy/5 flex gap-5 items-start">
             <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shrink-0 shadow-sm border border-brand-navy/5">
                <Info className="text-brand-gold w-5 h-5 flex items-center" />
             </div>
             <div>
               <p className="text-[10px] font-black uppercase tracking-widest text-brand-navy mb-1 leading-none">Authorization Protocol</p>
      ...
      <Modal isOpen={isRepayModalOpen} onClose={() => setIsRepayModalOpen(false)} title="Treasury Repayment">
        <form onSubmit={handleRepay} className="space-y-10">
           <div className="p-10 bg-brand-ivory rounded-[3rem] border border-brand-gold/10 shadow-inner">
              <div className="space-y-8">
                <div className="flex justify-between items-center text-sm font-bold text-brand-navy pb-4 border-b border-brand-gold/10">
                  <span>Current Exposure:</span>
                  <span className="text-red-600 text-lg">₹{repayData.max.toLocaleString()}</span>
                </div>
                <div className="space-y-3">
           ...
           <button 
             disabled={processing}
             type="submit"
             className="w-full bg-brand-navy/95 hover:bg-brand-navy text-white py-6 rounded-full font-bold text-xs uppercase tracking-[0.25em] shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-3"
           >
             {processing ? <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto"></div> : (
               <>
