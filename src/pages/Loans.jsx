...
const Loans = () => {
  ...
  const handleRepay = async (loanId) => {
    ...
    try {
      ...
      await financeService.repayLoan(loanId, Number(amount))
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Underwriting Authorization">
        <form onSubmit={handleIssueLoan} className="space-y-10">
           ...
           <button 
             disabled={processing}
             type="submit"
             className="w-full heritage-gradient text-white py-6 rounded-full font-bold text-xs uppercase tracking-[0.25em] shadow-2xl hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-3"
           >
