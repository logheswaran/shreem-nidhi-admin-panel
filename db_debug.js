...
async function run() {
  const { data, error } = await supabase.from('ledger').select('*').limit(50);
  const result = {
    error,
    ledgerTypes: data ? data.map(d => ({ 
        id: d.id, 
        tt: d.transaction_type, 
        rt: d.reference_type,
        amount: d.amount 
    })) : []
  };
  fs.writeFileSync('ledger_debug.json', JSON.stringify(result, null, 2));
}
